...
- **시드 스크립트**: `scripts/seed-cms.ts`(러너 tsx)로 1회 시드 완료. 콘텐츠 진실은 이제
  Supabase DB이며 admin에서 직접 편집하므로, 재실행 시 편집 내용이 원복되는 사고를 막기 위해
  스크립트·`pnpm seed:cms`(`:apply`) 자체를 저장소에서 제거했다. (시드 결과: artists 44 ·
  releases 21 · tours 8)
