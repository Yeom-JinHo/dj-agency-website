          ...
          >
            ...
            <TableHeader className="[&_th]:bg-background [&_th]:sticky [&_th]:top-0 [&_th]:z-10 [&_th]:shadow-[inset_0_-1px_0_var(--border)] [&_tr]:border-b-0">
              <TableRow>
                ...
                {/* 잉여 폭 흡수용 스페이서 컬럼.
                    표는 w-full(ui/table.tsx)이고 table-layout이 auto라, 컬럼 폭 합이 컨테이너보다
                    좁으면 브라우저가 남는 폭을 반드시 어딘가로 나눠준다 — 폭을 지정하지 않은 컬럼이
                    있으면 그쪽이 전부 먹고, 전부 지정돼 있으면 모든 컬럼에 비례 배분된다. 즉 "남는
                    폭을 표 오른쪽에 그냥 남긴다"는 선택지는 CSS에 없다. 잉여가 갈 곳을 명시하지
                    않으면 유일한 무제한 컬럼(제목·이름 = 행의 앵커)이 전부 흡수한다.
                    실측(2560 뷰포트): 아티스트 이름 컬럼 1950px, 이름 글자가 끝나는 x=436과 다음
                    컬럼 시작 x=2335 사이가 약 1900px 공백 — 한 행의 좌우 정보가 화면 양 끝으로
                    갈라져 눈으로 이을 수 없었다. 정보가 0인 자리가 폭을 먹는다는 점에서, 이건 폭
                    상한으로 고치려던 문제와 같은 종류다.
                    그래서 잉여를 받을 자리를 마지막에 하나 만든다. w-full(width:100%)은 auto 레이아웃
                    에서 "남는 건 전부 이 컬럼"이라는 뜻이라, 나머지 컬럼은 각자의 지정 폭 또는 내용
                    폭에 머문다. 결과적으로 데이터 컬럼 배분이 뷰포트와 무관해지고(1440·1920·2560이
                    동일) 화면이 넓어질수록 커지는 건 이 빈 칸뿐이다.
                    ...
                    - 앵커에도 상한을 주고 잉여를 표 끝에 남기기: 위 이유로 CSS가 그렇게 두지 않는다.
                      모든 컬럼에 비례 배분돼 이 커밋 이전(장소 326px)의 배분으로 되돌아간다.
                    - 앵커에 max-width: table-layout auto의 컬럼 폭 계산은 셀의 max-width를 보지 않는다. */}
