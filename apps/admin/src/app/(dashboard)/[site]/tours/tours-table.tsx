...
const columns: DataTableColumn<TourRow>[] = [
  ...
  },
  {
    ...
    // 지난 공연은 제목에서 '강조'만 거둔다(굵기 500→400, foreground→muted-foreground).
    ...
    // 아니라 '강조를 뺀 것'이라는 데 있다 — 같은 행의 아티스트·장소·일시가 이미 쓰는 바로
    // 그 톤이라, 지난 행은 본문 레벨로 평평해지고 예정 행만 제목이 도드라진다.
    //
    // 기각한 안들:
    // - '예정' 뱃지에 primary: 신규 사이트는 전 행이 예정이라 100% 인디고가 되어 세팅
    //   초기에 액센트가 신호이길 그친다. 게다가 이 앱에서 primary는 '행동 가능·현재 위치'
    //   (CTA·정렬 활성·포커스 링)를 뜻해 상태 라벨에 쓰면 어휘가 흐려지고, 상태 컬럼이
    //   인디고+amber+red+회색 4색이 된다. 예정은 평범한 기본값인데 브랜드색을 얻고
    //   매진·취소가 경고색이면 색 위계가 실제 중요도와 어긋난다.
    ...
    cell: (row) =>
      row.isPast ? (
        <span className="text-muted-foreground font-normal">{row.title}</span>
      ) : (
        row.title
      ),
...
export function ToursTable({
  ...
  return (
    <DataTable
      ...
      //
      // isPast는 이제 두 곳이 나눠 나른다 — 상태 컬럼의 '지남' 뱃지가 의미(스크린리더 포함)를,
      // 제목 셀의 강조 해제가 스캔을 맡는다. 행 단위 처리로 돌아가지 말 것: 행 전체를 건드리면
      // 이미 muted-foreground인 아티스트·장소·일시가 AA 아래로 떨어진다. 강조를 가진 셀은
      // 제목뿐이라, 손댈 곳도 제목뿐이다.
