...
// with zero dependency on the ~350KB country GeoJSON — the points are
// projected server-side.
...
export function WorldMap({
  ...
  // the compact `pointsFlat` array in the RSC payload — instead of also as a
  // giant pre-rendered SVG string. Gating on `mounted` keeps SSR and the first client
  ...
  const { width, height, pointsFlat } = mapData;
  const svg = useMemo(
    () => (mounted ? pointsToSvg(pointsFlat, width, height, dotColor) : ""),
    [mounted, pointsFlat, width, height, dotColor],
  );
