...
export function Header() {
  ...
  return (
    // VFL 헤더와 동일 기법: bg-white/80 + backdrop-blur-sm 반투명 프로스티드 바.
    // 스크롤 콘텐츠가 뒤로 비치던 문제를 흰 베일+블러로 가린다(하드 밴드/스크림 대신).
    <header className="fixed inset-x-0 top-0 z-20 bg-white/80 backdrop-blur-sm">
      <div className="flex h-16 items-center px-6 md:h-20 md:px-10">
        {/* 전 라우트(홈 포함) 공통 좌상단 홈 복귀 앵커. */}
        <Link
          href="/"
